...
class Decoder(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask,image_embed):
        ...
        concatenated_output = torch.cat([encoder_output, image_embed], dim=2)

        # Apply linear projection
        #projection_layer = nn.Linear(concatenated_output.shape[2], encoder_output.shape[2])
        encoder_output = self.projection_layer(concatenated_output)
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings,
        ...
        self.vision_model = BlipForQuestionAnswering.from_pretrained("Salesforce/blip-vqa-base").vision_model
    ...
    def encode(self, src, src_mask,pixel_values=None):
        # (batch, seq_len, d_model)
        src = self.src_embed(src)
        src = self.src_pos(src)
        image_embed = self.vision_model(pixel_values)['last_hidden_state']
        return self.encoder(src, src_mask),image_embed

    def decode(self, encoder_output: torch.Tensor, src_mask: torch.Tensor, tgt: torch.Tensor, tgt_mask: torch.Tensor,image_embedding=None):
        # (batch, seq_len, d_model)
        tgt = self.tgt_embed(tgt)
        tgt = self.tgt_pos(tgt)
        return self.decoder(tgt, encoder_output, src_mask, tgt_mask,image_embedding)
...
def build_transformer(vocab_size: int,  src_seq_len: int, tgt_seq_len: int, d_model: int = 512,
    ...
    return transformer
# build_transformer(src_vocab_size=5, )

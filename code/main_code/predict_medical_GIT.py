...
GIT_folder = generated_result_folder + os.sep + 'GIT'
current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
if not os.path.exists(generated_result_folder):
    os.mkdir(generated_result_folder)
if not os.path.exists(GIT_folder):
    os.mkdir(GIT_folder)
generated_result_excel_file = f"{GIT_folder}{os.sep}test_data_{current_time}.xlsx"
...
def model_definition():
    model = BlipForQuestionAnswering.from_pretrained("Model/git-saved-model").to("cuda")

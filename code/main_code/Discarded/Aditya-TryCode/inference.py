...
def main(config_path: Text, img_path: Text, question: Text) -> None:
    ...
    checkpoint = os.path.join(config["train"]["output_dir"], config["model"]["name"], config["inference"]["checkpoint"], "training_args.bin")
    model.load_state_dict(torch.load("~//.cache//huggingface//hub//models--microsoft--beit-base-patch16-224-pt22k-ft22k//snapshots//9da301148150e37e533abef672062fa49f6bda4f//pytorch_model.bin"))
    model.to(device)

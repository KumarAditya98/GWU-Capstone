...
class BertModel(BertPreTrainedModel):
    """
    The models can behave as an encoder (with only self-attention) as well as a decoder, in which case a layer of
    ...
    def get_extended_attention_mask(self, attention_mask: Tensor, input_shape: Tuple[int], device: device,
        ...
        Arguments:
            ...
            device: (:obj:`torch.device`):
                The device of the input to the models.
    ...
    def forward(
        ...
        encoder_hidden_states  (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
            ...
            the models is configured as a decoder.
        encoder_attention_mask (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            Mask to avoid performing attention on the padding token indices of the encoder input. This mask is used in
            the cross-attention if the models is configured as a decoder. Mask values selected in ``[0, 1]``:
            - 1 for tokens that are **not masked**,
            - 0 for tokens that are **masked**.
        past_key_values (:obj:`tuple(tuple(torch.FloatTensor))` of length :obj:`config.n_layers` with each tuple having 4 tensors of shape :obj:`(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
            Contains precomputed key and value hidden states of the attention blocks. Can be used to speed up decoding.
            If :obj:`past_key_values` are used, the user can optionally input only the last :obj:`decoder_input_ids`
            (those that don't have their past key value states given to this models) of shape :obj:`(batch_size, 1)`
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> models = BertLMHeadModel.from_pretrained('bert-base-cased', config=config)
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None, attention_mask=None, **model_kwargs):
        ...
        # if models is used as a decoder in encoder-decoder models, the decoder attention mask is created on the fly
        if attention_mask is None:
